    ...
    <div class="page">
      <div class="page-header">
        <div>
          <h1>Plantillas</h1>
          <p class="subtitle">
            Universales para todos, y personales solo para vos
          </p>
        </div>
        @if (canCreate()) {
      ...
      @if (globalTemplates().length) {
        <section class="section">
          <h2 class="section-title">Universales</h2>
          <div class="template-list">
            @for (tpl of globalTemplates(); track tpl.id) {
              <div
                class="card template-card"
                [class.has-theme]="
                  !!tpl.backgroundColor || !!tpl.backgroundImageUrl
                "
      ...
      @if (personalTemplates().length || canCreate()) {
        <section class="section">
          <h2 class="section-title">
            {{ isAdmin() ? 'Personales' : 'Mis plantillas' }}
          </h2>
          <div class="template-list">
            @for (tpl of personalTemplates(); track tpl.id) {
              ...
              <div class="empty-state card">
                <strong>Sin plantillas personales</strong>
                @if (canCreate()) {
                  Creá una para usarla al iniciar una retrospectiva.
                }
              </div>
            }
          </div>
...
export class TemplatesPage implements OnInit {
  ...
  readonly globalTemplates = computed(() =>
    this.templates().filter((t) => t.isGlobal),
  );
  readonly personalTemplates = computed(() =>
    this.templates().filter((t) => !t.isGlobal),
  );
  ...
  canManage(tpl: Template) {
    return this.isAdmin() || tpl.createdById === this.auth.user()?.id;
  }
